...
export default function PaymentPage() {
        ...
        return (
          <div key={s.key} className="flex items-center">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
              done ? 'bg-emerald-50 text-emerald-700' : active ? 'bg-primary-50 text-primary-700' : 'bg-gray-100 text-gray-400'
            }`}>
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                done ? 'bg-emerald-500 text-white' : active ? 'bg-primary-600 text-white' : 'bg-gray-300 text-white'
              }`}>
                {done ? '✓' : s.num}
              </span>
      ...
      <div className="max-w-lg mx-auto animate-fadeIn">
        ...
        <div className="min-h-[50vh] flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500 mb-4">Securing your transaction...</p>
    ...
    return (
      <div className="max-w-lg mx-auto animate-fadeIn">
        ...
        <Card className="text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">A receipt has been generated for your records.</p>
          ...
          <div className="bg-gray-50 rounded-xl p-5 text-left mb-6 border border-gray-200">
            ...
            <div className="space-y-2.5">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-mono text-xs text-gray-900">{completedPayment.transaction_id}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Invoice</span>
                <span className="font-mono text-xs text-gray-900">{completedPayment.invoice_number?.slice(0, 18)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Service</span>
                <span className="text-gray-900">{completedPayment.service_name}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Plan</span>
                <span className="text-gray-900">{completedPayment.plan_name}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Subtotal</span>
                <span className="text-gray-900">${completedPayment.amount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Tax (18%)</span>
                <span className="text-gray-900">${completedPayment.tax.toFixed(2)}</span>
              </div>
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-900">{new Date(completedPayment.paid_at).toLocaleString()}</span>
              </div>
              {completedPayment.card_brand && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Card</span>
                  <span className="text-gray-900">{getBrandName(completedPayment.card_brand)} •••• {completedPayment.card_last4}</span>
                </div>
              )}
            </div>
          </div>
    ...
    return (
      <div className="max-w-lg mx-auto animate-fadeIn">
        ...
        <Card className="text-center">
          ...
          <p className="text-sm text-gray-500 mb-4">Your payment was not processed.</p>
          ...
          <div className="text-center mb-6">
            {otpResendTimer > 0 ? (
              <p className="text-xs text-gray-400">
                Resend code in <span className="font-medium text-gray-600">{otpResendTimer}s</span>
              </p>
            ) : (
              <button
                onClick={() => {
            ...
            {selectedInvoice && (
              <Card className="border-primary-200">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Service</span>
                    <span className="text-gray-900">{selectedInvoice.service_name}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Plan</span>
                    <span className="text-gray-900">{selectedInvoice.plan_name}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Subtotal</span>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full pl-11 pr-24 py-2.5 border rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary-500 ${formErrors.cardNumber ? 'border-red-300' : 'border-gray-300'}`}
                    />
                    <CreditCard className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 flex gap-1">
                      {['visa', 'mastercard', 'amex'].map(b => (
                        <span key={b} className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${brand === b ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-400'}`}>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <select value={expiryMonth} onChange={e => setExpiryMonth(e.target.value)} className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm bg-white">
                  ...
                  <div>
                    ...
                    <select value={expiryYear} onChange={e => setExpiryYear(e.target.value)} className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm bg-white">
                  ...
                  <div>
                    ...
                    <input type="password" value={cvv} onChange={e => setCvv(e.target.value.replace(/\D/g, '').slice(0, 4))} onFocus={() => setIsFlipped(true)} onBlur={() => setIsFlipped(false)} placeholder="•••" maxLength={4}
                      className={`w-full px-3 py-2.5 border rounded-lg text-sm font-mono ${formErrors.cvv ? 'border-red-300' : 'border-gray-300'}`} />
                  </div>
                </div>
                {formErrors.expiry && <p className="text-xs text-red-600 -mt-2">{formErrors.expiry}</p>}

                <div className="pt-2 border-t">
                  <p className="text-sm font-medium text-gray-900 mb-3">Billing Address</p>
                  <div className="space-y-3">
                    ...
                    <select value={billingCountry} onChange={e => setBillingCountry(e.target.value)} className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm bg-white">
                ...
                <label className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl cursor-pointer">
                  <input type="checkbox" checked={saveCard} onChange={e => setSaveCard(e.target.checked)} className="w-4 h-4 rounded border-gray-300 text-primary-600" />
                  <div>
      ...
      <div className="max-w-2xl mx-auto animate-fadeIn">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">For {selectedInvoice?.service_name} — {selectedInvoice?.plan_name} · ${selectedInvoice?.total.toFixed(2)}</p>
          </div>
        </div>
              ...
              {savedMethods.map(m => (
                <button key={m.id} onClick={() => { setPaymentType('saved'); setSelectedSavedId(m.id); }}
                  className={`w-full text-left p-4 rounded-xl border-2 transition-all flex items-center gap-4 ${
                    paymentType === 'saved' && selectedSavedId === m.id ? 'border-primary-500 bg-primary-50' : 'border-gray-200 hover:border-gray-300'
                  }`}>
                  <div className="p-2.5 bg-white rounded-lg border">
                    <CreditCard className="w-5 h-5 text-gray-600" />
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-gray-900">{getBrandName(m.brand)} {m.label}</p>
                    <p className="text-xs text-gray-500">Expires {String(m.expiry_month).padStart(2, '0')}/{m.expiry_year} · {m.holder_name}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    {m.is_default && <Badge variant="info">Default</Badge>}
                    {paymentType === 'saved' && selectedSavedId === m.id && <CheckCircle className="w-5 h-5 text-primary-600" />}
                  </div>
                </button>
              ))}
            </div>
        ...
        {/* Other methods */}
        <h3 className="text-sm font-semibold text-gray-700 mb-3">Other Payment Methods</h3>
        <div className="grid gap-3 mb-6">
          {[
            ...
            <button key={pm.type} onClick={() => { setPaymentType(pm.type); setSelectedSavedId(''); }}
              className={`w-full text-left p-4 rounded-xl border-2 transition-all flex items-center gap-4 ${
                paymentType === pm.type && !selectedSavedId ? 'border-primary-500 bg-primary-50' : 'border-gray-200 hover:border-gray-300'
              }`}>
              <div className={`p-2.5 rounded-lg border ${paymentType === pm.type && !selectedSavedId ? 'bg-primary-100 text-primary-600 border-primary-200' : 'bg-gray-50 text-gray-500 border-gray-200'}`}>
                {pm.icon}
              </div>
              <div>
              ...
              {paymentType === pm.type && !selectedSavedId && <CheckCircle className="w-5 h-5 text-primary-600 ml-auto" />}
            </button>
          ))}
        </div>
  ...
  if (step === 'select' && activeTab === 'pay') {
    return (
      <div className="max-w-2xl mx-auto animate-fadeIn">
        <StepWizard />
        <h2 className="text-lg font-bold text-gray-900 mb-1">Select Invoice to Pay</h2>
        <p className="text-sm text-gray-500 mb-6">{pendingInvoices.length} pending invoice{pendingInvoices.length !== 1 ? 's' : ''}</p>
          ...
          <div className="space-y-3">
            {pendingInvoices.map(inv => (
              <button key={inv.id} onClick={() => handleSelectInvoice(inv)}
                className="w-full text-left p-4 rounded-xl border-2 border-gray-200 hover:border-primary-300 hover:bg-primary-50/50 transition-all group">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-gray-900 group-hover:text-primary-700">{inv.service_name}</p>
                      <p className="text-xs text-gray-500">{inv.plan_name} · <span className="font-mono">{inv.invoice_number.slice(0, 18)}</span></p>
                    </div>
                  </div>
        ...
        <div className="flex items-center justify-center gap-6 mt-8 py-3 border-t">
          {[
            ...
            <div key={i} className="flex items-center gap-1.5 text-xs text-gray-400">{item.icon} {item.text}</div>
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="animate-fadeIn">
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg mb-6 max-w-xl">
        {[
          ...
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            ...
            {tab.count > 0 && <span className={`px-1.5 py-0.5 rounded-full text-xs ${activeTab === tab.key ? 'bg-primary-100 text-primary-700' : 'bg-gray-200'}`}>{tab.count}</span>}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'pay' && (
        <div className="max-w-2xl mx-auto">
          <h2 className="text-lg font-bold text-gray-900 mb-1">Select Invoice to Pay</h2>
          <p className="text-sm text-gray-500 mb-6">{pendingInvoices.length} pending</p>
          {pendingInvoices.length === 0 ? (
                ...
                <button key={inv.id} onClick={() => handleSelectInvoice(inv)}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-900">{inv.service_name}</p>
                        <p className="text-xs text-gray-500">{inv.plan_name} · <span className="font-mono">{inv.invoice_number.slice(0, 18)}</span></p>
                      </div>
                    </div>
                    <div className="text-right">
            ...
            <div className="space-y-3">
              {savedMethods.map(m => (
                <Card key={m.id} className={m.is_default ? 'border-primary-200' : ''}>
                  <div className="flex items-center gap-4">
                    <div className="p-3 bg-gray-50 rounded-xl border"><CreditCard className="w-6 h-6 text-gray-600" /></div>
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-semibold text-gray-900">{getBrandName(m.brand)} {m.label}</p>
                        {m.is_default && <Badge variant="info">Default</Badge>}
                      </div>
                      <p className="text-xs text-gray-500 mt-0.5">{m.holder_name} · Exp {String(m.expiry_month).padStart(2, '0')}/{m.expiry_year}</p>
                    </div>
                  ...
                  <tbody>
                    {paymentHistory.map(p => (
                      <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                        <td className="py-3 px-4"><p className="text-sm font-mono text-gray-700">{p.transaction_id.slice(0, 22)}</p><p className="text-xs text-gray-400 font-mono">{p.invoice_number?.slice(0, 16)}</p></td>
                        <td className="py-3 px-4 text-sm text-gray-700">{p.service_name}<p className="text-xs text-gray-400">{p.plan_name}</p></td>
                        <td className="py-3 px-4 text-sm font-medium text-gray-900">${p.total.toFixed(2)}</td>
                        <td className="py-3 px-4 text-sm text-gray-700">{p.method}</td>
                        <td className="py-3 px-4 text-sm text-gray-500">{new Date(p.paid_at).toLocaleDateString()}</td>
