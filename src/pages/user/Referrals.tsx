  ...
  return (
    <div className="animate-fadeIn max-w-3xl">
      ...
      <Card className="bg-gradient-to-r from-primary-600 to-purple-600 border-0 text-white mb-6">
        ...
        <Button onClick={handleShare} className="gap-2 bg-white text-primary/90 hover:bg-white/90">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <Card className="text-center">
          ...
          <p className="text-2xl font-bold text-foreground">{info.referred_count}</p>
          <p className="text-xs text-muted-foreground dark:text-slate-300">Friends Invited</p>
        </Card>
        <Card className="text-center">
          <DollarSign className="w-6 h-6 text-emerald-500 mx-auto mb-2" />
          <p className="text-2xl font-bold text-foreground">${info.credits_earned}</p>
          <p className="text-xs text-muted-foreground dark:text-slate-300">Credits Earned</p>
        </Card>
        <Card className="text-center">
          <Star className="w-6 h-6 text-amber-500 mx-auto mb-2" />
          <p className="text-2xl font-bold text-foreground">${info.credits_balance}</p>
          <p className="text-xs text-muted-foreground dark:text-slate-300">Available Credits</p>
        </Card>
      </div>
        ...
        <div className="grid sm:grid-cols-3 gap-6">
          {[
            ...
            <div key={i} className="text-center">
              <div className="w-12 h-12 bg-primary-50 dark:bg-primary-900/30 text-primary rounded-xl flex items-center justify-center mx-auto mb-3">{s.icon}</div>
              <h4 className="font-semibold text-foreground mb-1">{s.title}</h4>
              <p className="text-xs text-muted-foreground dark:text-slate-300">{s.desc}</p>
            </div>
          ))}
        </div>
      </Card>
        ...
        {info.referrals.length === 0 ? (
          ...
          <div className="space-y-2">
            {info.referrals.map((r, i) => (
              <div key={i} className="flex items-center justify-between p-3 bg-muted/50 dark:bg-slate-700/50 rounded-lg">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-primary-100 dark:bg-primary-900/40 text-primary rounded-full flex items-center justify-center text-sm font-bold">
                    {r.email[0]?.toUpperCase()}
                  </div>
                  <div>
                    <p className="text-sm font-medium text-foreground">{r.email}</p>
                    <p className="text-xs text-muted-foreground/80 dark:text-slate-400">{new Date(r.date).toLocaleDateString()}</p>
                  </div>
                </div>
